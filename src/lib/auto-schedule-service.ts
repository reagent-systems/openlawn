...
export class AutoScheduleGenerator {
  ...
  private async createCrewSchedule(
    ...
    // TSP optimization using Distance Matrix API
    const optimizedRoute = await this.optimizeRoute(customers);
   ...
   * TSP optimization using Distance Matrix API
   */
  private async optimizeRoute(customers: Customer[]): Promise<{ lat: number; lng: number }[]> {
    ...
    try {
      ...
      console.log(`🎯 Auto-schedule TSP optimized route: ${customers.length} customers reordered to minimize ${optimizationResult.totalDistance.toFixed(1)} miles (individual round-trip from 1467 Creeks Edge Ct)`);

      // Return the optimized path
      return optimizationResult.optimizedPath;

    } catch (error) {
      console.error('Error using TSP optimization in auto-schedule, falling back to nearest neighbor:', error);
